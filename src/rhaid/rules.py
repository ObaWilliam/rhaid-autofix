...
FORCE_DEBUG_STDERR = os.environ.get("RHAID_DEBUG_STDERR", "").lower() in ("1", "true", "yes")
...
def rule(id: str):
    ...
    def deco(fn):
        ...
        if not suppress_debug:
            print(f"[rhaid.rules] Registering rule: {id} from {fn.__module__}.{fn.__name__}", file=sys.stderr)
...
def r_tabs(path: str, content: str, ctx: dict) -> List[RuleResult]:
    ...
    for i, ln in enumerate(content.splitlines(), 1):
        idx = ln.find("\t")
        if idx != -1:
            out.append(RuleResult("format:tabs", "Tabs detected; prefer spaces.", "info", path, line=i, col=idx + 1))
            if len(out) >= 10:
                break
    return out
...
def r_spacing(path: str, content: str, ctx: dict) -> List[RuleResult]:
    ...
    for i, line in enumerate(content.splitlines(), 1):
        ...
        if m:
            col = m.start() + 1
            out.append(RuleResult("format:spacing", "Extra spaces around assignment '='.", "info", path, line=i, col=col))
...
def r_json_object(path: str, content: str, ctx: dict) -> List[RuleResult]:
    ...
    for i, line in enumerate(content.splitlines(), 1):
        ...
        debug_print(f"[json:object] checking line {i}: '{line.rstrip()}' is_comment={is_comment}")
        if not is_comment:
            continue
        if "{" in s and "}" in s and ":" in s:
            col = line.find("{") + 1
            out.append(RuleResult("json:object", "JSON-like object embedded in a comment.", "info", path, line=i, col=col))
