...
def _extract_top_import_block(text: str):
    ...
    def is_import(s):
        ss = s.strip()
        return ss.startswith("import ") or ss.startswith("from ")

    while i < len(lines) and (
        lines[i].startswith("#!") or re.match(r"^#.*coding[:=]", lines[i])
    ):
        i += 1
    start = i
    saw = False
...
def _sort_import_lines(lines):
    im = [line for line in lines if line.strip().startswith(("import ", "from "))]
    other = {i for i, line in enumerate(lines) if line not in im}
...
def r_py_imports_order(path, content, ctx):
    ...
    if block != _sort_import_lines(block):
        return [
            RuleResult(
                "py:imports_order",
                "Top import block can be normalized (sorted).",
                "info",
                path,
                line=s + 1,
                col=1,
...
def r_py_unused_import(path, content, ctx):
    ...
    debug_print(
        f"[TRACE py:unused_import] path={path} endswith .py={path.lower().endswith('.py')}"
    )
    if not path.lower().endswith(".py"):
    ...
    for it in imports:
        ...
        if unused and len(unused) == len(it.names):
            ...
            issues.append(
                RuleResult(
                    "py:unused_import",
                    f"Unused import names: {', '.join(unused)}",
                    "warning",
                    path,
                    line=s + it.line_idx + 1,
                    col=1,
