...
def f_tf_spacing(path, content, issues, ctx):
    ...
    fx = "".join(out)
    return FixResult(
        fx != content, ["Normalized Terraform '=' spacing to 'key = value'."], fx
    )


@rule("tf:parse")
def r_tf_parse(path, content, ctx):
    if not path.lower().endswith(".tf") or not hcl2:
        return []
    try:
        hcl2.loads(content)
        return []
