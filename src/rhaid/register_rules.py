...
def register_all_rules():
	...
	js_mod = None
	try:
		js_mod = importlib.import_module('rhaid.js_rules')
	except Exception:
		pass
	# Python AST rules
	...
	debug_print(f"[DEBUG register_all_rules] Registered rules: {list(rules_mod._RULES.keys())}")
	debug_print(f"[DEBUG register_all_rules] Registered fixers: {list(rules_mod._FIXERS.keys())}")
